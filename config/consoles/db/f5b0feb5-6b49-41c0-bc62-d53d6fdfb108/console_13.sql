     ...
     --    INNER JOIN
       ...
       --     and d.city_name like concat("%", '深圳', "%")
        ...
        or task.assignee_id_ = '70000000060556')
             AND task.status_ != 'TRANSFORMING') a join biz_apply_order b ON (
#         a.proc_inst_id_ = b.flow_instance_id
           a.parent_inst_id_ = b.flow_instance_id
          )
   INNER JOIN
 	s_area_info d on b.branch_id = d.company_code
        LEFT join biz_isr_mixed isr on b.apply_no = isr.apply_no
 ...
 LEFT JOIN SYS_TYPE
                  type ON type.id_ = a.type_id_
     --  order by task.PRIORITY_ DESC, CREATE_TIME_ DESC
